...
REPORT_DIR = BASE_DIR / "attendance_reports"
...
def get_attendance_log_path(target_date):
...
def normalize_attendance_df(df):
    """
    Chuẩn hóa log điểm danh để tương thích cả header cũ và header mới.
    """
    ...
    # Header cũ có thể là "status"; header mới là "attendance_status"
    if "attendance_status" not in df.columns and "status" in df.columns:
        df["attendance_status"] = df["status"]
    ...
    # parse timestamp để sort/deduplicate
    df["timestamp_dt"] = pd.to_datetime(df["timestamp"], errors="coerce")

    # Nếu cùng 1 người bị ghi nhiều dòng do file cũ, lấy lần đầu trong ngày
    df = df.sort_values("timestamp_dt", na_position="last")
    df = df.drop_duplicates(subset=["person_id"], keep="first")

    return df


def load_attendance_log(target_date):
    log_path = get_attendance_log_path(target_date)
    ...
    try:
        raw_df = pd.read_csv(log_path, encoding="utf-8-sig")
    except pd.errors.EmptyDataError:
        raw_df = pd.DataFrame()

    return normalize_attendance_df(raw_df), log_path


def build_attendance_report(target_date, active_only=True):
    """
    Ghép people.csv với attendance log theo ngày.
    Output:
    - PRESENT nếu person_id có trong log ngày đó
    - ABSENT nếu không có
    """
    people = load_people_df().copy()

    if len(people) == 0:
        return pd.DataFrame(), get_attendance_log_path(target_date)

    for col in ["person_id", "full_name", "student_code", "class_name", "status"]:
        if col not in people.columns:
            people[col] = ""

    people["person_id"] = people["person_id"].astype(str)

    if active_only and "status" in people.columns:
        people = people[people["status"].astype(str).str.lower() == "active"].copy()

    log_df, log_path = load_attendance_log(target_date)

    if len(log_df) > 0:
        log_df["person_id"] = log_df["person_id"].astype(str)
    ...
    for col in keep_log_cols:
        if col not in log_df.columns:
            log_df[col] = None

    merged = people.merge(
        log_df[keep_log_cols],
        on="person_id",
        how="left"
    )

    merged["attendance_result"] = np.where(
    ...
    return merged[output_cols], log_path
...
def dataframe_to_excel_bytes(df, sheet_name="attendance_report"):
    output = BytesIO()
...
tab_dashboard, tab_people, tab_register, tab_search, tab_analysis, tab_attendance = st.tabs([
    "Dashboard",
    "Quản lý CSDL khuôn mặt",
    "Đăng ký người mới",
    "Tìm kiếm / Xác minh",
    "Phân tích ảnh khuôn mặt",
    "Quản lý điểm danh / Report"
])
...
# ============================================================
# TAB 6: ATTENDANCE MANAGEMENT / REPORT
# ============================================================

with tab_attendance:
    st.subheader("Quản lý điểm danh / Xuất report")

    st.info(
        "Tab này đọc dữ liệu từ attendance_logs/attendance_YYYY-MM-DD.csv, "
        "ghép với people.csv để tạo danh sách PRESENT / ABSENT. "
        "Camera điểm danh realtime vẫn chạy bằng file webcam_attendance_poc.py."
    )

    col_filter_1, col_filter_2, col_filter_3 = st.columns([1, 1, 2])
    ...
    with col_filter_2:
        active_only = st.checkbox(
            "Chỉ tính người active",
            value=True
        )

    report_df, log_path = build_attendance_report(
        selected_date,
        active_only=active_only
    )

    st.markdown("### File log nguồn")
    st.code(str(log_path))

    total_count = len(report_df)
    present_count = int((report_df["attendance_result"] == "PRESENT").sum()) if total_count > 0 else 0
    absent_count = int((report_df["attendance_result"] == "ABSENT").sum()) if total_count > 0 else 0

    col_m1, col_m2, col_m3, col_m4 = st.columns(4)
    col_m1.metric("Tổng số người", total_count)
    col_m2.metric("Đã điểm danh", present_count)
    col_m3.metric("Vắng", absent_count)
    present_rate = (present_count / total_count * 100) if total_count > 0 else 0
    col_m4.metric("Tỷ lệ có mặt", f"{present_rate:.1f}%")

    if not log_path.exists():
        st.warning(
            "Chưa có file log điểm danh cho ngày này. "
            "Nếu muốn có dữ liệu, hãy chạy webcam_attendance_poc.py để điểm danh trước."
        )

    if len(report_df) == 0:
        st.warning("people.csv đang rỗng hoặc không có người phù hợp với bộ lọc.")
    else:
        st.markdown("### Bộ lọc hiển thị")

        col_view_1, col_view_2 = st.columns([1, 2])
        ...
        if status_filter != "ALL":
            view_df = view_df[view_df["attendance_result"] == status_filter].copy()

        if keyword:
            search_text = (
                view_df["person_id"].astype(str) + " " +
                view_df["full_name"].astype(str) + " " +
                view_df["student_code"].astype(str) + " " +
                view_df["class_name"].astype(str)
            ).str.lower()

            view_df = view_df[search_text.str.contains(keyword, na=False)].copy()

        st.markdown("### Báo cáo điểm danh")

        def highlight_attendance(row):
            if row["attendance_result"] == "PRESENT":
                # xanh đậm, chữ sáng
                return [
                    "background-color: #14532d; color: #f8fafc; font-weight: 600;"
                ] * len(row)
            elif row["attendance_result"] == "ABSENT":
                # đỏ đậm, chữ sáng
                return [
                    "background-color: #7f1d1d; color: #f8fafc; font-weight: 600;"
                ] * len(row)
            else:
                return [""] * len(row)
        ...
        styled_view_df = (
            view_df.style
            .apply(highlight_attendance, axis=1)
            .format({
                "similarity": "{:.4f}",
                "face_real_score": "{:.4f}",
                "full_real_score": "{:.4f}",
        ...
        report_filename_csv = f"attendance_report_{date_str}.csv"
        report_filename_xlsx = f"attendance_report_{date_str}.xlsx"

        st.markdown("### Xuất report")

        col_d1, col_d2, col_d3 = st.columns(3)

        with col_d1:
            st.download_button(
                label="Tải report CSV",
                data=dataframe_to_csv_bytes(report_df),
                file_name=report_filename_csv,
                mime="text/csv"
            )

        with col_d2:
            try:
                excel_bytes = dataframe_to_excel_bytes(report_df)
                st.download_button(
                    label="Tải report Excel",
                    data=excel_bytes,
                    file_name=report_filename_xlsx,
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
            except Exception as e:
                st.warning(f"Không tạo được Excel: {e}")

        with col_d3:
            if st.button("Lưu report vào thư mục attendance_reports"):
                out_path = REPORT_DIR / report_filename_csv
                report_df.to_csv(out_path, index=False, encoding="utf-8-sig")
                st.success(f"Đã lưu report: {out_path}")

        st.markdown("### Danh sách vắng")
        absent_df = report_df[report_df["attendance_result"] == "ABSENT"].copy()
        ...
        if len(absent_df) > 0:
            ...
            st.success("Không có người vắng trong danh sách lọc hiện tại.")
