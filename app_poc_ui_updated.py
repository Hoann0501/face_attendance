...
def load_attribute_model():
    ...
    Nếu chưa có file model thì trả None để UI vẫn chạy được.
    """
    ...
    checkpoint = torch.load(ATTR_MODEL_PATH, map_location=device)
    ...
    meta = {
        ...
        "val_age_acc": checkpoint.get("val_age_acc", None),
    }
...
def predict_face_attribute(face_pil, attr_model, attr_transform, attr_meta):
    ...
    return {
        ...
        "age_group": idx_to_age_group.get(age_idx, str(age_idx)),
...
if attr_meta is not None:
    ...
    if attr_meta.get("val_age_acc") is not None:
        st.sidebar.write(f"Age group val acc: {attr_meta['val_age_acc']:.4f}")
...
with tab_dashboard:
    ...
    status_df = pd.DataFrame([
        ...
        },
        {
            ...
            "status": "POC",
            "evidence": "detect face, bbox, score, landmark, embedding, gender, age_group"
        },
        {
...
with tab_analysis:
    ...
    st.info(
        ...
        "Kết quả age_group là nhóm tuổi, không phải tuổi chính xác từng năm."
    )
